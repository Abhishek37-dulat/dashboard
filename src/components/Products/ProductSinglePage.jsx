...
const BoxforPreImg = styled(Box)(({ theme }) => ({
  // border: "1px solid black",
  width: "100%",
  display: "flex",
  flexDirection: "row",
...
const AddProduct = () => {
  ...
  const sizeSubmit = (e) => {
    ...
    if (sizeType.length <= 0) {
      ...
      setSizeAll([...sizeAll, { size: sizeInput, type: sizeType[0] }]);
    }
  };
  ...
  const handleMouseEnterImage = (e, index) => {
    setPreLoadedImagesBoolean(true);
    setPreLoadedImagesIndex(index);
    // preImageDisplay.current.style.display = "flex";
  };
  const handleMouseLeaveImage = (e, index) => {
    setPreLoadedImagesBoolean(false);
    setPreLoadedImagesIndex("");
  ...
  const firstDataFetch = async () => {
    setConstantValues({
      ...
      productdescription: singleProduct?.product_description,
